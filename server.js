...
app.get('/api/admin/payment-confirmations', authenticateToken, isAdmin, async (req, res) => {
    try {
        // คำนวณวันที่ 5 วันข้างหน้า (ใช้ UTC เพื่อความแม่นยำ)
        ...
        // ดึง installment ที่ due_date อยู่ระหว่างวันนี้ถึง 5 วันข้างหน้า และยังไม่ชำระ
        // ใช้ >= สำหรับวันนี้ (รวมวันนี้) และ <= สำหรับ 5 วันข้างหน้า
        const [installments] = await pool.execute(
            `SELECT 
                ip.installment_id,
                ip.order_id,
