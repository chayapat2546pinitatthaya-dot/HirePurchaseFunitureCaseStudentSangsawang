...
const initializeDatabase = async () => {
    ...
    if (!connected) {
        console.error('Cannot initialize database schema. Server will continue but database operations may fail.');
        return;
    }
    
    try {
        await ensureCustomerAltAddressesColumn();
        await ensureCustomerEmailVerificationColumns();
        await ensureOrderShippingAddressColumn();
 ...
 * 
 * @param {number} totalAmount - ราคารวมทั้งหมด
 * @param {number} periods - จำนวนงวด
 * @returns {Array<number>} Array ของจำนวนเงินแต่ละงวด
 */
...
app.get('/api/admin/notification-summary', authenticateToken, isAdmin, async (req, res) => {
    try {
        ...
        // ใช้ช่วงวันนี้ถึงอีก 5 วันเหมือนหน้าแจ้งเตือนการชำระเงิน
        ...
        const [paymentRows] = await pool.execute(
            `SELECT 
                MAX(installment_id) AS latest_installment_id,
        ...
        res.json({
            ...
            latestPaymentId: Number(paymentRow.latest_installment_id) || 0,
            pendingPayments: Number(paymentRow.pending_payments) || 0
        });
    } catch (error) {
...
app.post('/api/orders', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // Calculate total - ใช้ราคาเงินสดเสมอ (ไม่ว่าจะส่งราคาอะไรมา)
        let cashTotal = 0;
        ...
        for (const item of items) {
            ...
            // หาราคาเงินสด (โปรโมชันก่อน ถ้าไม่มีใช้ราคาปกติ)
            let cashPrice = product.price_cash_promo || product.price_cash;
            if (cashPrice == null) {
                cashPrice = product.price_cash;
            }
            
            // ใช้ราคาเงินสดเสมอ (ไม่ใช้ item.price เพราะอาจเป็นราคาผ่อนชำระ)
            const unitCashPrice = cashPrice || 0;
            cashTotal += unitCashPrice * item.quantity;
            
            // เก็บ unitPrice สำหรับบันทึกใน order_detail (ใช้ราคาเงินสด)
            resolvedItems.push({ ...item, unitPrice: unitCashPrice });
        }
        ...
        // คำนวณยอดรวมที่ถูกต้อง: ถ้าเป็นผ่อนชำระ ให้คำนวณจากราคาเงินสด + ดอกเบี้ย
        let totalAmount = cashTotal;
        if (isInstallmentMethod && periods > 1) {
            const rate = getInstallmentRateByMonths(periods);
            totalAmount = Math.round(cashTotal * (1 + rate));
        }
        
        // คำนวณจำนวนเงินแต่ละงวดให้ถูกต้องตามหลักบัญชี
        const installmentAmounts = isInstallmentMethod && periods > 1
            ? calculateInstallmentAmounts(totalAmount, periods)
            : [totalAmount];
        
        // ใช้จำนวนเงินงวดแรกเป็น monthly_payment สำหรับแสดงผล (แต่จริงๆ แต่ละงวดอาจไม่เท่ากัน)
        const monthlyPayment = installmentAmounts.length > 0 ? installmentAmounts[0] : totalAmount;
        ...
        if (isInstallmentMethod && periods > 0) {
            // ดึง order_date จาก order ที่เพิ่งสร้าง
            const [orderRows] = await connection.execute(
                'SELECT order_date FROM `order` WHERE order_id = ?',
                [orderId]
            );
            const orderDate = orderRows[0]?.order_date ? new Date(orderRows[0].order_date) : new Date();
            
            for (let i = 0; i < installmentAmounts.length; i++) {
                // คำนวณ due_date จาก order_date แทน new Date()
                ...
                await connection.execute(
                    ...
                    [orderId, i + 1, installmentAmounts[i], dueDateStr, 'pending']
                );
            }
        }
...
app.put('/api/orders/approve/:id', authenticateToken, isAdmin, async (req, res) => {
    ...
    try {
        ...
        // ตรวจสอบ order และ installment_payments
        const [orders] = await connection.execute(
            'SELECT order_id, installment_periods, payment_method, order_status FROM `order` WHERE order_id = ?',
            [orderId]
        );
        ...
        if (isInstallmentMethod && order.installment_periods > 1) {
            const approvalDate = new Date(); // วันที่อนุมัติ
            const [installments] = await connection.execute(
                'SELECT installment_id, installment_number FROM installment_payments WHERE order_id = ? ORDER BY installment_number',
                [orderId]
            );
            ...
            for (let i = 0; i < installments.length; i++) {
                ...
                dueDate.setMonth(dueDate.getMonth() + i + 1); // งวดแรก = เดือนถัดไป
                
                // อัปเดต due_date เท่านั้น (ไม่ตั้งเป็นชำระแล้ว - ต้องรอแอดมินกดปุ่มชำระเงินแล้ว)
                await connection.execute(
                    `UPDATE installment_payments 
                     SET payment_due_date = ? 
                     WHERE installment_id = ?`,
                    [dueDate.toISOString().split('T')[0], installment.installment_id]
                );
            }
        }
        ...
        // ส่งอีเมลแจ้งลูกค้า (ถ้ามี)
        try {
            const [orderRows] = await pool.execute(
                `SELECT o.*, c.customer_email, c.customer_fname, c.customer_lname 
                 FROM \`order\` o 
                 JOIN customer c ON o.customer_id = c.customer_id 
                 WHERE o.order_id = ?`,
                [orderId]
            ...
            if (orderRows.length > 0) {
                ...
                const emailPayload = buildAwaitingPaymentEmail(orderData, orderData);
            ...
            // ไม่ throw error เพราะการส่งอีเมลไม่ควรทำให้ approve ล้มเหลว
        }
        
        res.json({
            message: 'คำสั่งซื้อได้รับการอนุมัติแล้ว',
            status: 'approved'
        });
    } catch (error) {
        await connection.rollback();
...
// Mark installment payment as paid (Admin only)
app.put('/api/orders/:orderId/installments/:installmentId/mark-paid', authenticateToken, isAdmin, async (req, res) => {
    ...
    try {
        ...
        // ตรวจสอบว่า installment นี้เป็นของ order นี้จริง
        const [installments] = await connection.execute(
            'SELECT installment_id, order_id, payment_status FROM installment_payments WHERE installment_id = ? AND order_id = ?',
        ...
        if (installments.length === 0) {
            await connection.rollback();
            connection.release();
            return res.status(404).json({ error: 'ไม่พบข้อมูลงวดการชำระเงิน' });
        }
        ...
        if (installment.payment_status === 'paid') {
            await connection.rollback();
            connection.release();
            return res.status(400).json({ error: 'งวดนี้ชำระแล้ว' });
        }
        
        // อัปเดตสถานะเป็นชำระแล้ว และบันทึกวันที่ชำระ
        ...
        await connection.execute(
            `UPDATE installment_payments 
             SET payment_status = 'paid', 
                 payment_date = ?
             WHERE installment_id = ?`,
            [paymentDate.toISOString().split('T')[0], installmentId]
        );
        ...
        res.status(500).json({ error: 'ไม่สามารถอัปเดตสถานะการชำระเงินได้' });
    } finally {
        connection.release();
    }
});

// Get upcoming installment payments (within 5 days) - Admin only
app.get('/api/admin/payment-confirmations', authenticateToken, isAdmin, async (req, res) => {
    try {
        // อนุญาตเลือกช่วงวันที่เอง ถ้าไม่ระบุให้ default = วันนี้ ถึง 5 วันข้างหน้า
        ...
        const endDateCandidate = parseDate(req.query.end_date, defaultEnd);
        const endDate = endDateCandidate < startDate ? startDate : endDateCandidate;

        const startStr = startDate.toISOString().split('T')[0];
        const endStr = endDate.toISOString().split('T')[0];
        const status = (req.query.status || 'pending').toLowerCase();
        const isHistory = status === 'paid' || status === 'history';

        // ถ้าเป็นประวัติที่จ่ายแล้ว ใช้ payment_date; ถ้าเป็นรายการรอยืนยัน ใช้ payment_due_date
        const dateField = isHistory ? 'ip.payment_date' : 'ip.payment_due_date';
        const dateCondition = `${dateField} >= ? AND ${dateField} <= ?`;
        const statusCondition = isHistory ? `ip.payment_status = 'paid'` : `ip.payment_status != 'paid'`;
        const orderSort = isHistory ? 'ip.payment_date DESC, ip.order_id ASC' : 'ip.payment_due_date ASC, ip.order_id ASC';
        ...
        const [installments] = await pool.execute(
            `SELECT 
             ...
             ORDER BY ${orderSort}, ip.installment_number ASC`,
            [startStr, endStr]
        );
        
        const formattedInstallments = installments.map(inst => {
            if (!inst || !inst.installment_id) {
                return null;
            }
            ...
            return {
                ...
                installment_amount: Number(inst.installment_amount) || 0,
                payment_due_date: inst.payment_due_date,
                ...
                order: {
                    ...
                    installment_periods: inst.installment_periods,
                    shipping_address: normalizeShippingRecord(inst.shipping_address)
                },
                customer: {
                    customer_id: inst.customer_id,
                    customer_fname: inst.customer_fname || '',
                    customer_lname: inst.customer_lname || '',
                    customer_tel: inst.customer_tel || '',
...
const startServer = async () => {
  ...
  if (!dbConnected) {
    console.warn('Warning: Database connection failed. Server will start but database operations may fail.');
